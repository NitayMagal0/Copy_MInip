...
class LightsTests {
    ...
    /**
     * Test for multiple light sources on sphere - combines directional, point, and spot lights
     */
    @Test
    void sphereMultipleLights() {
        // Use scene1 (no ambient light) to see the combined effect clearly
        scene1.geometries.add(sphere);

        // Add three different light sources with different colors and positions
        scene1.lights.add(new DirectionalLight(new Color(300, 0, 0), new Vector(1, 1, -0.5))); // Red directional (like the sun so no need for setK values)
        scene1.lights.add(new PointLight(new Color(0, 300, 0), new Point(-100, 100, 50)) // Green point
                .setKl(0.0005).setKq(0.0001));
        scene1.lights.add(new SpotLight(new Color(0, 0, 300), new Point(100, -100, 50), new Vector(-1, 1, -1)) // Blue spot
                .setKl(0.0001).setKq(0.00005));

        camera1.setResolution(500, 500)
                .build()
                .renderImage()
                .writeToImage("sphere Multiple Lights");
    }

    /**
     * Test for multiple light sources on triangles - combines directional, point, and spot lights
     * Purpose- show how multiple lights interact with flat surfaces and create combined illumination patterns
     */
    @Test
    void trianglesMultipleLights() {
        // Use scene2 (with ambient light)
        scene2.geometries.add(triangle1, triangle2);

        // Add three different light sources from different directions
        scene2.lights.add(new DirectionalLight(new Color(800, 0, 0), new Vector(-1, -1, -2))); // Red directional
        scene2.lights.add(new PointLight(new Color(0, 800, 0), new Point(50, 50, -50)) // Green point
                .setKl(0.001).setKq(0.0002));
        scene2.lights.add(new SpotLight(new Color(0, 0, 800), new Point(-50, -50, -50), new Vector(1, 1, -1)) // Blue spot
                .setKl(0.0005).setKq(0.0001));

        camera2.setResolution(500, 500)
                .build()
                .renderImage()
                .writeToImage("triangles Multiple Lights");
    }
}
